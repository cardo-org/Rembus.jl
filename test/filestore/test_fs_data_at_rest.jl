...
try
    ...
    @error "[fs_data_at_rest] error: $e"
    @test false
finally
    shutdown()
    df = Rembus.data_at_rest(broker=broker_name, from=Dates.Second(20))
    @test nrow(df) == rounds * 2 + 4 # two connection_up and two connection_down
    @info "[fs_data_at_rest] end"
end
