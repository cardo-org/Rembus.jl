...
function mytopic(ctx, rb)
    @info "[test_component_qos2]: message mytopic received"
    ctx.count += 1
end
...
@info "[test_component_qos2] start"
try
    mkpath(Rembus.rembus_dir())
    run()
catch e
    @error "[test_component_qos2] error: $e"
    @test false
finally
    shutdown()
end

@info "[test_component_qos2] stop"
